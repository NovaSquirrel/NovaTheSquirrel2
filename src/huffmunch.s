...
; Edited by NovaSquirrel for 65c816 use
.include "snes.inc"
.smart
.import huffmunch_zpblock

; in: X = stream index, hm_node = pointer to data block
; out: A = output byte length of current stream, hm_node = total stream count in data
; preserves register size
.export huffmunch_load_snes

; out: reads 1 byte from stream, result in A (X,Y,flags clobbered)
; preserves register size
.export huffmunch_read_snes

; out: reads 2 bytes from stream, result in A (X,Y,flags clobbered)
; preserves register size
.export huffmunch_word_snes
...
.segment "ZEROPAGE"
.globalzp hm_node, hm_stream, hm_tree, hm_byte, hm_status, hm_length
hm_node:   .res 3 ; pointer to current node of tree
hm_stream: .res 3 ; pointer to bitstream
...
.proc huffmunch_load
    ...
    ; Use the same bank on the other pointer
    lda hm_node+2
    sta hm_stream+2
	...
	stz hm_byte ; hm_byte doesn't need initialization, just for consistency
	stz hm_status
	stz hm_length
	rtl
.endproc
